...
void lv_port_disp_init(void)
{
    ...
    // static uint8_t buf_1_1[MY_DISP_HOR_RES * 30 * BYTE_PER_PIXEL];            /*A buffer for 10 rows*/
...
{
    // HAL_DMA_StateTypeDef spi_dma_tx_status;
    // spi_dma_tx_status = HAL_DMA_GetState(&hdma_spi1_tx);    // 获取DMA发送状态

    // if (spi_dma_tx_status == HAL_DMA_STATE_READY)
    ...
    //     }
    // else if(spi_dma_tx_status == HAL_DMA_STATE_BUSY)
    //     ;
    // else
    //     disp_disable_update();
