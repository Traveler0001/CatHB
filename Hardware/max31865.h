...
00h	        配置寄存器	            可读可写    D7	  BIAS（偏置电压控制）	    0：禁用 VBIAS（低功耗模式）；1：启用 VBIAS
                                              D6	 转换模式	              0：单次转换模式（需通过 D5 触发）；1：自动连续转换模式
                                              D5	 1-Shot（单次转换触发）	   1：在单次模式下启动一次转换（转换后自动清零）
                                              D4	 3-Wire（RTD 接线模式）	  0：2 线或 4 线模式；1：3 线模式（用于消除引线电阻影响）
                                              D3-D2	 故障检测周期	          00：无自动检测；01：自动检测（每转换一次）；10：每 4 次转换检测一次；11：每 8 次转换检测一次
                                              D1	 故障状态清零	          1：清除故障寄存器（07h）的所有故障标志（需同时将 D5、D3、D2 置 0）
                                              D0     50/60Hz 滤波选择	      0：60Hz 滤波（适用于美洲电网）；1：50Hz 滤波（适用于欧洲 / 亚洲电网）
01h	        RTD 电阻 MSB 寄存器	    只读	   D7-D0  RTD 电阻数据高 8 位	    存储 RTD 电阻测量值的高 8 位（与 02h 的 D7-D1 组成 15 位有效数据）
02h	        RTD 电阻 LSB 寄存器	    只读	   D7-D1  RTD 电阻数据低 7 位	    存储 RTD 电阻测量值的低 7 位（与 01h 共同组成 15 位数据，分辨率为 R_REF/32768）
                                              D0	 FAULT（故障标志）	      0：无故障；1：检测到 RTD 相关故障（需结合 07h 寄存器查看详情）
03h	        高故障门限 MSB 寄存器	 可读可写	D7-D0  高故障门限高 8 位	     存储 RTD 电阻上限阈值的高 8 位（用于故障检测）
04h	        高故障门限 LSB 寄存器	 可读可写	D7-D0  高故障门限低 8 位	     存储 RTD 电阻上限阈值的低 8 位（仅高 7 位有效，D0 无意义）
05h	        低故障门限 MSB 寄存器	 可读可写	D7-D0  低故障门限高 8 位	     存储 RTD 电阻下限阈值的高 8 位（用于故障检测）
06h	        低故障门限 LSB 寄存器	 可读可写	D7-D0  低故障门限低 8 位	     存储 RTD 电阻下限阈值的低 8 位（仅高 7 位有效，D0 无意义）
07h	        故障状态寄存器	         只读	    D7	   高温故障（RTD > 高门限）	 1：RTD 电阻值超过高故障门限
                                              D6	 低温故障（RTD < 低门限）	1：RTD 电阻值低于低故障门限
                                              D5	 RTD 输入开路故障	       1：检测到 RTD 开路
...
#define MAX31865_CONFIG_MODE_ONESHOT 1         // 0x00[6:1]: 单次转换模式（需通过 D5 触发）
#define MAX31865_CONFIG_1SHOT 1                // 0x00[5:1]: 单次模式下启动一次转换（转换后自动清零）
...
#define MAX31865_CONFIG_FAULT_DETECT 1         // 0x00[1:1]: 清除故障寄存器（07h）的所有故障标志（需同时将 D5、D3、D2 置 0）
...
extern MAX31865_Config max_config;   // MAX31865配置寄存器
extern MAX31865_Status_List max_status_list;   // MAX31865故障状态寄存器
extern MAX31865_RtdData max_rtdData; // MAX31865 RTD数据结构体
extern MAX31685_STATUS max_status;

MAX31685_STATUS MAX31865_Init(void);
MAX31685_STATUS MAX31865_Read(void);
uint8_t MAX31865_ReadConfig(void);
uint8_t MAX31865_SetConfig(void);
